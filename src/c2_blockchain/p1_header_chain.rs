...
impl Header {
    ...
    fn verify_sub_chain(&self, chain: &[Header]) -> bool {
        ...
        for header in chain {
            if header.parent != hash(current) || header.height != current.height + 1 {
                return false;
            }
            current = header;
        }
...
fn build_an_invalid_chain() -> Vec<Header> {
    ...
    // Create an invalid block (e.g., wrong parent hash or wrong height)
    let mut invalid_header = current.child();
    invalid_header.height = 5; // Invalid height
